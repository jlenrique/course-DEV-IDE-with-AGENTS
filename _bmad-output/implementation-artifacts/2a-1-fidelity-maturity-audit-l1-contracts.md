...
- [ ] Task 1: Conduct the formal fidelity maturity audit (AC: #1, #2, #3, #4, #5, #6)
  - [ ] 1.1 Read all pipeline artifact schemas (lesson plan template, slide brief template, context envelope, narration script template, segment manifest template, review protocol, checkpoint coordination, source bundle format)
  - [ ] 1.2 For each gate G0–G6, evaluate L1 Contracts pillar: does a written, testable fidelity criterion exist? Evidence: cite specific file + field
  - [ ] 1.3 For each gate, evaluate L2 Evaluation pillar: does something independently assess output fidelity? Evidence: cite evaluator or note absence
  - [ ] 1.4 For each gate, evaluate L3 Memory pillar: does the system capture fidelity outcomes for learning? Evidence: cite sidecar fields or note absence
  - [ ] 1.5 For each gate, evaluate Perception pillar: can the evaluator actually interpret the artifact modality? Evidence: cite sensory bridge or note blindness
  - [ ] 1.6 Compile audit into `_bmad-output/implementation-artifacts/fidelity-audit-baseline-2026-03-28.md`

- [ ] Task 2: Produce the Leaky Neck report (AC: #7, #8, #9)
...
- [ ] Task 3: Create L1 fidelity contract directory and schema (AC: #10, #11)
  ...
  - [ ] 3.2 Create `state/config/fidelity-contracts/_schema.yaml` defining the standard contract format
  - [ ] 3.3 Create a contract validation script `scripts/validate_fidelity_contracts.py` that checks structural completeness of all contract files

- [ ] Task 4: Define L1 contracts for G0–G2 (AC: #12, #13, #14)
  - [ ] 4.1 Create `state/config/fidelity-contracts/g0-source-bundle.yaml` with criteria derived from `skills/source-wrangler/references/bundle-format.md`
  - [ ] 4.2 Create `state/config/fidelity-contracts/g1-lesson-plan.yaml` with criteria derived from `skills/bmad-agent-content-creator/references/template-lesson-plan.md`
  - [ ] 4.3 Create `state/config/fidelity-contracts/g2-slide-brief.yaml` with criteria derived from `skills/bmad-agent-content-creator/references/template-slide-brief.md`

- [ ] Task 5: Define L1 contracts for G3–G4 (AC: #15, #16)
  - [ ] 5.1 Create `state/config/fidelity-contracts/g3-generated-slides.yaml` with criteria derived from `skills/bmad-agent-gamma/references/context-envelope-schema.md` and slide brief template
  - [ ] 5.2 Create `state/config/fidelity-contracts/g4-narration-script.yaml` with criteria derived from narration script template and segment manifest template

- [ ] Task 6: Define L1 contracts for G5–G6 (AC: #17, #18)
  - [ ] 6.1 Create `state/config/fidelity-contracts/g5-audio.yaml` with criteria derived from ElevenLabs audio skill references
  - [ ] 6.2 Create `state/config/fidelity-contracts/g6-composition.yaml` with criteria derived from compositor skill references
...
- [ ] Task 8: Validate all deliverables (AC: #21, #22, #23)
  ...
  - [ ] 8.2 Verify audit report evidence references exist in the repository
  - [ ] 8.3 Verify architectural references are consistent with GOLD document definitions
  - [ ] 8.4 Update sprint-status.yaml: `2a-1-fidelity-maturity-audit-l1-contracts: done`
