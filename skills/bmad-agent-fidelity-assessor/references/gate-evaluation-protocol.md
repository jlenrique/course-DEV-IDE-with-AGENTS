...
- **G4-18 (No new concepts in interstitials):** Interstitial narration must not introduce concept-bearing terms absent from the head segment's narration or its resolvable `source_ref` scope. This is enforced with a deterministic concept-token heuristic and reported per interstitial.
...
- **Bridge language enforcement note:** Spoken intro/outro/pivot cue enforcement remains part of the deterministic Pass 2 validator that supports G4 policy adherence. Treat missing learner-heard bridge language as concrete evidence for a G4-12 policy drift finding rather than as a standalone numbered criterion.
