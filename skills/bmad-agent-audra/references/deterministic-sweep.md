...
3. **Pipeline manifest comment:** Confirm the `SYNC-WITH:` line above `PIPELINE_STEPS` references an existing canonical pipeline source-of-truth (post-Epic-33: `state/config/pipeline-manifest.yaml`; pre-Epic-33 packs pointed at the prompt-pack file directly). If the SSOT is renamed, versioned, or migrated, update the comment and `PIPELINE_STEPS` together or record an L1 finding, type: `alteration`, severity: `high`, check: `hud-pipeline-lockstep`. Strict enforcement of SSOT chain is delegated to `scripts/utilities/check_pipeline_manifest_lockstep.py`; this L1-10 check verifies only that the SYNC-WITH target resolves to an existing file.
