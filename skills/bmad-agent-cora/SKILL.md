...
| HZ | Harmonization run: scoped coherence sweep (operator chooses scope — full repo / since last handoff / specific directory); invokes Audra L1 then L2. **HUD scope union:** every sweep unions `run_hud.py`, `progress_map.py`, `tests/test_run_hud.py`, and `tests/test_progress_map.py` into Audra’s change window (see harmonization protocol) so the Run HUD stays in lockstep with sprint status, pipeline steps, and dev-cycle feeds. **Workflow-stage lockstep is mandatory:** stage additions/renames (for example `04A`) must be present in prompt packs, operator cards, structural-walk manifests, and Marcus workflow templates before closure. | Load `./references/harmonization-protocol.md` |
