...
**Maintenance:** update at (a) each Epic retrospective close, (b) each story closure that names a new follow-on, (c) any session-wrapup where the operator flags a new deferred item. Last refreshed: **2026-04-24** (Cora `/harmonize` cycle — added parameter-lockstep cleanup as named follow-on; pending Winston ruling).
...
| **Parameter directory ↔ schema lockstep cleanup** | 2026-04-24 Cora full-repo `/harmonize` (reports/dev-coherence/2026-04-24-1732/) — surfaced as L1-3 finding | Operator decision after Winston ruling | 21 entries in `docs/parameter-directory.md` lack matching rows in `state/config/parameter-registry-schema.yaml` (12 run-constants + 9 narration-time + 3 profiles.*.slide_role granularity divergence). Today's `tests/test_parameter_registry_schema.py` validates only the schema's own structural integrity, not the bidirectional contract. **Two-step routing:** (a) Winston rules whether the missing rows are derived/computed and intentionally schema-exempt, or genuine misses; (b) Paige applies the lockstep edit (substantial doc/schema work). Natural home: small Winston+Paige story (1-3 pt) that simultaneously ships planned `scripts/utilities/check_parameter_lockstep.py` per Audra SKILL.md External Skills Phase 3, closing the CI invariant gap. Could bundle META-CLAUDEMD + L1-10-INFO stale-pointer fixes if those weren't landed inline (this session landed them inline 2026-04-24). |

**Total named follow-ons: 14.**
