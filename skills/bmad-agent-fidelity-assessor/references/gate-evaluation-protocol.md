...
- **G4-10 (Runtime plan coherence):** Segment runtime decisions align with the approved runtime plan controls (`target_total_runtime_minutes`, per-slide targets, average runtime assumptions, and variance policy). Unjustified drift is a failure.
...
- **G4-16 (Spoken bridge language vs bridge_type):** When `bridge_type` is `intro`, `outro`, or `both`, `narration_text` must contain at least one substring from the configured `pedagogical_bridging.spoken_bridge_policy` intro/outro pattern lists (see `validate-irene-pass2-handoff.py`). For `both`, both intro-class and outro-class cues must appear. Enforcement follows `spoken_bridge_policy.enforcement` (`off` | `warn` | `error`); `warn` escalates under strict runtime policy. Failure is a high-severity deterministic finding when enforcement demands it.
