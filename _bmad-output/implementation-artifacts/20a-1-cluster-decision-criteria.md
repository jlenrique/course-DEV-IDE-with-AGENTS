...
- [ ] Task 1: Define the decision criteria clearly (AC: 1-4)
  - [ ] 1.1: Define `concept density` in terms of how many distinct explanatory beats a topic naturally contains
  - [ ] 1.2: Define `visual complexity` in terms of whether the current slide would benefit from decomposition, isolation, or reduction
  - [ ] 1.3: Define `pedagogical weight` in terms of concept centrality to the learning objective and learner burden if left undersupported
  - [ ] 1.4: Define `operator input` as bounded override or preference, not sole authorship of cluster selection
- [ ] Task 2: Produce a reusable decision framework reference for Irene (AC: 1-6)
  - [ ] 2.1: Add a new reference document under `skills/bmad-agent-content-creator/references/`
  - [ ] 2.2: Include pass/fail examples of likely cluster heads versus slides that should stay flat
  - [ ] 2.3: Include explicit cues for when one interstitial is enough versus when 2-3 are justified
- [ ] Task 3: Tie the framework to the C1M1 MVP (AC: 6)
  - [ ] 3.1: Document that the first proof seeks exactly 3 clusters for beginning, middle, and end
  - [ ] 3.2: Note that the initial stop point is Storyboard A, so criteria should optimize for visual proof before narration concerns
- [ ] Task 4: Update Irene-facing references (AC: 5-6)
  - [ ] 4.1: Update [SKILL.md](C:/Users/juanl/Documents/GitHub/course-DEV-IDE-with-AGENTS/skills/bmad-agent-content-creator/SKILL.md) to reference the cluster decision framework in the future Pass 1 cluster-planning step
  - [ ] 4.2: Cross-link to [delegation-protocol.md](C:/Users/juanl/Documents/GitHub/course-DEV-IDE-with-AGENTS/skills/bmad-agent-content-creator/references/delegation-protocol.md) only where the criteria need to shape future writer briefs
