...
2. Complete Gate 1 and approve Irene Pass 1 artifacts.
3. Build the pre-dispatch bundle:
   - `g2-slide-brief.md`
...
4. Verify every slide has exactly one fidelity mode.
5. For each literal-visual card, provide exactly one dispatch-ready source:
   - `image_url`, or
   - `preintegration_png_path` in tracked/default mode only
6. If any `preintegration_png_path` is used, include `site_repo_url`.
7. Produce the Gate 6B packet at `<bundle-dir>/literal-visual-operator-packet.md`.
8. Do not dispatch Prompt 7 until all required literal-visual cards are operator-ready.
9. Present the pre-dispatch summary, get explicit approval, and run Gary mixed-fidelity dispatch.
10. Validate the dispatch result with `validate-gary-dispatch-ready.py`.
11. Generate Storyboard A from the dispatch payload and get explicit Gate 2 approval.
12. Write `authorized-storyboard.json`.
13. If `DOUBLE_DISPATCH` is enabled:
...
14. If `MOTION_ENABLED` is enabled:
...
15. Run Irene Pass 2 using approved `gary_slide_output`.
16. Regenerate Storyboard B and get approval before downstream audio/script finalization.
17. Save a compact run receipt with outcomes, blockers, and next action.
